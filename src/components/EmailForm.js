...
export default class EmailForm extends React.Component {
  ...
  validateDOB(date) {
    ...
    if (today >= new Date(birthDate.getFullYear() + 18, birthDate.getMonth(), birthDate.getDate()) && birthDate.getFullYear().toString().length === 4) {
        return true;
    } else {
        return false;
    }
  ...
  onFormSubmit(event) {
    ...
    if(this.validateEmail(email) && this.validateName(firstName) && this.validateName(lastName) && this.validateDOB(dob) && this.validateAmount(amount) && this.validateCurrency(currency)) {
      this.setState({
        formValid: "valid",
        statusShown: true
      });
    ...
    if(this.validateEmail(email) && this.validateName(firstName) && this.validateName(lastName) && this.validateDOB(dob) && this.validateAmount(amount) && this.validateCurrency(currency)) {
      ...
      .then((space) => space.createEntry('emailContacts', {
        fields: {
          email: {
            'en-US': email
          },
      ...
      .then((entry) => {
        ...
        //     method: 'post',
        //     // url: 'http://localhost/ughh-premier-handlers/skratchMailHandler.php', // mailChimpScript
        //     url: "https://undergroundhiphop.com/ughh-premier-handlers/skratchMailHandler.php",
        //     data: {
        //         mode: "",
        //         userRecord: {
        //             email: email,
  ...
  onInputChange(event) {
    ...
    setTimeout(() => {
      if(this.state.emailValid === "valid" && this.state.firstNameValid === "valid" && this.state.lastNameValid === "valid" && this.state.dobValid === "valid" && this.state.amountValid === "valid" && this.state.currencyValid === "valid") {
        this.setState({
          formValid: "valid"
        });
      } else {
  ...
  render() {
    ...
    return (
      <div className="email-form-container">
        <button className={`email-form-container__form-button`} onClick={this.toggleModal}>Sign Up</button>
        <div className={`email-form-container__modal ${(this.state.modalShown ? "modal-shown" : "")}`}>
          <form className="email-form" autoComplete="off">
            ...
            <div className="form-row">
              <input type="text" className="email-input" placeholder="Email" value={this.state.emailField} onChange={this.onInputChange} />
              <div className={`validation-message ${(this.state.emailValid === "invalid" ? "validation-shown" : "")}`}>
                <p>Please provide a valid email address</p>
              </div>
            </div>
            <div className="form-row">
              <input type="text" className="first-name-input" placeholder="First name" value={this.state.firstName} onChange={this.onInputChange} />
              <div className={`validation-message ${(this.state.firstNameValid === "invalid" ? "validation-shown" : "")}`}>
                <p>Please provide your first name</p>
              </div>
            </div>
            <div className="form-row">
              <input type="text" className="last-name-input" placeholder="Last name" value={this.state.lastName} onChange={this.onInputChange} />
              <div className={`validation-message ${(this.state.lastNameValid === "invalid" ? "validation-shown" : "")}`}>
                <p>Please provide your last name</p>
              </div>
            </div>
            <div className="form-row">
              <input type="text" className={`dob-input ${(this.state.dobValid !== "valid" ? "dob-input-invalid" : "")}`} placeholder="Date of birth (mm/dd/yyyy)" value={this.state.dob} onChange={this.onInputChange} onKeyUp={this.addSlashes} />
              <div className="input-format-helper">
                <p>In the format mm/dd/yyyy. For example, 01/30/1990 for January 30th, 1990. We'll add the slashes for you.</p>
              </div>
            ...
            <div className="form-row currency-questions">
              <input type="text" className={`amount-input ${(this.state.amountValid !== "valid" ? "amount-input-invalid" : "")}`} placeholder="Amount you'd like to spend" value={this.state.amount} onChange={this.onInputChange}/>
              <select name="currency" className="currency-input" onChange={this.onInputChange} value={this.state.currency}>
                <option value="default" disabled>{currencyPlaceholder}</option>
                <option value="usd">USD</option>
                <option value="bitcoin">Bitcoin</option>
                <option value="ethereum">Ethereum</option>
              </select>
              <div className={`validation-message ${(this.state.amountValid === "invalid" || this.state.currencyValid === "invalid" ? "validation-shown" : "")}`}>
                <p>Please let us know how much you're looking to spend on Skratch tokens</p>
              </div>

            </div>
            <button className={`email-button ${(this.state.formValid === "valid" ? "" : "button-disabled")}`} onClick={this.onFormSubmit}>{this.props.data.emailButton}</button>
            <div className="form-status">
              <img src={rollerImage.node.file.url} alt="Processing" className={(this.state.sendingData ? "status-shown" : "status-hidden")} />
              <img src={successImage.node.file.url} alt="Success" className={(this.state.success && this.state.statusShown ? "status-shown" : "status-hidden")} />
              <img src={failureImage.node.file.url} alt="Failure" className={(this.state.failure && this.state.statusShown ? "status-shown" : "status-hidden")} />
            </div>
